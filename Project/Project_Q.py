...
NUM_OBSERVATIONS = 1  # Number of frames to stack as input
...
def reward_function(info, prev_info):                                           #return reward for a given movement, ####weight needs tuning####
    ...
    if mario_world_y_prev > mario_world_y:                                        #The reward for jumping, might need to implemet checks for justified jumps
        jumping_reward = 0.2
    elif mario_world_y_prev > mario_world_y:
        jumping_reward = 0
    else:
        jumping_reward = -0.1
    ...
    reward = jumping_reward + living_reward
...
def train_agent():
    ...
    for episode in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS_PER_EPISODE):
            ...
            reward = reward_function(info, prev_info) * 2 + reward * 1                                       #Linear weighted sum of rewards, def needs tuning 
