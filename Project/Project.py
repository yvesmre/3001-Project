...
def make_action(screen, info, step, env, prev_action, prev_location, prev_block):
    ...
    action = rule_based_agent(info, mario_locations, enemy_locations, block_locations, item_locations, prev_action, prev_location, prev_block)

    prev_location = mario_locations
    prev_block = block_locations

    return action, prev_location, prev_block

################################################################################

def rule_based_agent(info, mario_locations, enemy_locations, block_locations, item_locations, prev_action, prev_location, prev_block):
    mario_world_x = info["x_pos"]
    mario_world_y = info["y_pos"]
    ...
    if prev_action == 1 or prev_action == None:
        if len(pipe_list) > 0 and len(enemy_list) > 0:                                  #if there is pipe ahead && if there are enemies ahead
            for mob in enemy_list:
                x, y = mob

                if abs(y - (pipe_list[0])[0][1]) < 5 and (pipe_list[0])[0][0] > x:      #if both mob and pipe are on the same level and mob is in front of pipe
                    if x - mario_x < 35 and x - mario_x > 0:                            #squash the mob if mob is in front of mario
                        return 2

                elif (pipe_list[0])[0][0] < x:                                          #if mob is behind the pipe
                    if (pipe_list[0])[0][0] - mario_x < 20 and (pipe_list[0])[0][0] - mario_x > 0:          #if pipe is in front of mario, jump
                        return 2                      
                    
                else:                                                                   #not enough distance
                    return 1

        elif len(enemy_list) > 0 and len(pipe_list) == 0:                               #if there no pipe ahead && if there are enemies ahead
            for mob in enemy_list:
                x, y = mob

                if x - mario_x < 35 and x - mario_x > 0 and abs(y - mario_y) < 5:       #squash the mob if mob is in front of mario and on the same level
                    return 2

                else:
                    return 1                                                            #not enough distance or not on the same level

        elif len(pipe_list) > 0 and len(enemy_list) == 0:                               #if there is pipe ahead && if there is no enemy ahead
            if (pipe_list[0])[0][0] - mario_x < 20 and (pipe_list[0])[0][0] - mario_x > 0:          #if pipe is in front of mario, jump
                return 2
    ...
    elif prev_action == 2 or prev_action == 5:                                                              #if the previous action is jump, this means that mario is either stomping on a mob, or jumping on top of a pipe
        location, dimensions, object_name = prev_location[0]
        mario_x_p, mario_y_p = location
        pipe_height = 0

        if len(prev_pipe_list) > 0:
            pipe_height = (prev_pipe_list[0])[1][1]

        if abs(mario_y_p - mario_y - pipe_height) < 5 and pipe_height != 0:             #check if mario has landed on a pipe successfully
            if len(enemy_list) > 0:                                                     #if there are mobs ahead
                for mob in enemy_list:
                    ...
                    elif abs(mario_y + pipe_height - y_co) > 5:                                 #if the mob is not on teh same level as mario, just move on to explore the next mob
                        continue
...
for step in range(100000):
    if obs is not None:
        action, prev_location, prev_block = make_action(obs, info, step, env, action, prev_location, prev_block)
    else:
        action = 1
