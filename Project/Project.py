...
def make_action(screen, info, step, env, prev_action, prev_location, prev_block, stuck_counter, stuck):
    ...
    if stuck == 1:
        ...
        stuck = 0
        action, stuck = rule_based_agent(info, mario_locations, enemy_locations, block_locations, item_locations, prev_action, prev_location, prev_block, stuck)
    ...
    return action, prev_location, prev_block, stuck_counter, stuck
        ...
        if count > 0 and len(enemy_list) > 0:                                  #if there is pipe ahead && if there are enemies ahead
            ...
            for mob in enemy_list:
                ...
                if x - mario_x < 55 and x - mario_x > 0 and abs(y - mario_y) < 5:       #squash the mob if mob is in front of mario and on the same level
                    ...
                    return 1, stuck                                                            #not enough distance or not on the same level
...
for step in range(100000):
    if obs is not None:
        action, prev_location, prev_block, stuck_counter, stuck = make_action(obs, info, step, env, action, prev_location, prev_block, stuck_counter, stuck)
